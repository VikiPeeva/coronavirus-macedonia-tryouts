...
def update_mk_covid_geojson(open_data_dir):
...
def update_mk_covid_datasets(open_data_dir): 
    ...
    healed_df.to_csv('{}mk/covid19/datasets/recovered_by_municipality.csv'.format(open_data_dir), index=False)
    ...
    dead_df.to_csv('{}mk/covid19/datasets/dead_by_municipality.csv'.format(open_data_dir), index=False)
    ...
    sick_hospitals.to_csv('{}mk/covid19/datasets/infected_by_hospital.csv'.format(open_data_dir), index=False)
    ...
    recovered_hospitals.to_csv('{}mk/covid19/datasets/recovered_by_hospital.csv'.format(open_data_dir), index=False)
    ...
    hospitals_geojson.to_file('{}mk/bolnici-hospitals/hospitals.geojson'.format(open_data_dir), 'GeoJSON')

def update_mk_covid_quarantine_geojson(open_data_dir):
    mk_municipalities = geopandas.read_file("https://raw.githubusercontent.com/klupp/opendata/master/mk/opstini-municipalities/municipalities_mq.geojson")
    mk_cities = mk_municipalities.dissolve(by='entity_mk', aggfunc='sum')                                
    # mk_cities['entity_mk'] = mk_cities.index
    mk_cities['id'] = mk_cities.index
    ...
    quarantine[['geometry', 'population']].to_file("{}mk/covid19/maps/covid-quarantine.geojson".format(open_data_dir), 'GeoJSON')
    

def update_mk_covid_summary_datasets(open_data_dir):
    infected_df = pd.read_csv("https://docs.google.com/spreadsheets/d/e/2PACX-1vR-Ul4NMiPvca7QH-RlYk2Q1hrVmjjGp5tr5n64l1z-SH5S2NoMeqjSd5Ulo171tHKM2Crfr7u0tpcz/pub?gid=0&single=true&output=csv", names=['city', 'infected_in', 'infected_in_city', 'date', 'count', 'age', 'source'], header=0)
    ...
    covid_mk.to_csv('{}mk/covid19/datasets/summary_by_municipality.csv'.format(open_data_dir))
    
def update_hospital_summary(open_data_dir):
    ...
    covid_hospitals.to_csv('{}mk/covid19/datasets/summary_by_hospital.csv'.format(open_data_dir))
    ...
    gdf.to_file("{}mk/covid19/maps/summary_hospital.geojson".format(open_data_dir), 'GeoJSON')
